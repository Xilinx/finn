module address_config #(
	...
	localparam int unsigned   AXIL_ADDR_BITS = $clog2((1 + (BASE_ADDR_BITS-1)/AXIL_DATA_BITS) * (AXIL_DATA_BITS/8))
) (
	input  logic                         aclk,
	input  logic                         aresetn,
	...
	uwire  cfg_en;
	uwire  cfg_we;
	uwire [BASE_ADDR_BITS-1:0]  cfg_d;
	uwire  cfg_rack;
	uwire [BASE_ADDR_BITS-1:0]  cfg_q;
	...
	axilite #(
		...
		.ip_en(cfg_en), .ip_wen(cfg_we), .ip_addr(), .ip_wdata(cfg_d),
		.ip_rack(cfg_rack), .ip_rdata(cfg_q)
	);

	// Base address register and read-back acknowledge
	logic [BASE_ADDR_BITS-1:0]  BaseAddr = '0;
	logic  RdAck = 0;
	always_ff @(posedge aclk) begin
		if(!aresetn) begin
			...
			if(cfg_en && cfg_we)  BaseAddr <= cfg_d;
			RdAck <= cfg_en && !cfg_we;
		end
	end
	assign	cfg_q = BaseAddr;
	assign	cfg_rack = RdAck;

	assign	base_address = BaseAddr;

endmodule : address_config

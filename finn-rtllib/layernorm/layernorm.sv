...
module layernorm #(
	...
	for(genvar  step = 0; step < 2; step++) begin : genNormDiamonds
		...
		fifo #(.DATA_WIDTH(SIMD*32), .DEPTH(VALUE_QUEUE_LEN + 1), .RAM_STYLE("shift")) bypass_queue (
			.clk, .rst,
		...
		if(1) begin : blkApply
			...
			if(1) begin : blkMeanCatcher
				uwire  norm_rdy;
				fifo #(.DATA_WIDTH(32), .DEPTH(STATS_QUEUE_LEN + 1), .RAM_STYLE("shift")) catcher (
					.clk, .rst,
			...
			// Output Queue
			uwire  rrdy;
			fifo #(.DATA_WIDTH(SIMD * 32), .DEPTH(CREDIT + 1), .RAM_STYLE("shift")) decouple (
				.clk, .rst,

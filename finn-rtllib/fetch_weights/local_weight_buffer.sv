...
module local_weight_buffer_double #(
	int unsigned  PE,
	int unsigned  SIMD,
...
endmodule : local_weight_buffer_double

module local_weight_buffer_single #(
	int unsigned  PE,
	int unsigned  SIMD,
	int unsigned  WEIGHT_WIDTH = 8,
	int unsigned  MH,
	...
	logic [PE-1:0][SIMD-1:0][WEIGHT_WIDTH-1:0]  Odat = '0;
	logic [PE-1:0][SIMD-1:0][WEIGHT_WIDTH-1:0]  odat_n;
	logic [PE-1:0][SIMD-1:0][WEIGHT_WIDTH-1:0]  odat_ram;

	logic [PE-1:0]                            a_we;
	logic [WGT_ADDR_BITS-1:0]                 a_addr;
	logic [SIMD-1:0][WEIGHT_WIDTH-1:0]        a_data_in;
	logic [WGT_ADDR_BITS-1:0]                 b_addr;
	logic                                     read_enable;

	assign  read_enable = ordy && (Full || (WrPntr > RdPntr));
	...
	always_comb begin
		...
		vld_s0_n = ordy? 0 : VldS0;
		vld_s1_n = ordy? VldS0 : VldS1;
		vld_n    = ordy? VldS1 : Vld;
		odat_n   = ordy? odat_ram : Odat;
		...
		if(ivld && irdy) begin
			...
			curr_pe_n = (CurrPe == PE-1)? 0 : CurrPe + 1;
			if(CurrPe == PE-1) begin
				wr_pntr_n = (WrPntr == N_TLS-1)? 0 : WrPntr + 1;
				if(WrPntr == N_TLS-1)
					full_n = 1;
			end
		end
		...
		if(read_enable) begin
			vld_s0_n = 1;
			rd_pntr_n = (RdPntr == N_TLS-1)? 0 : RdPntr + 1;
			if(RdPntr == N_TLS-1) begin
				reps_n = (Reps == N_REPS-1)? 0 : Reps + 1;
				if(Reps == N_REPS-1)
					full_n = 0;
			end
		end
	...
	for(genvar j = 0; j < PE; j++) begin : genPe
		if(N_TLS * WGT_WORD_BITS <= MAX_RAM_BITS) begin : genSingle
			...
			local_weight_ram #(
				...
				.clk(clk),
				.w_en(a_we[j]), .w_addr(a_addr), .w_data(a_data_in),
				.r_en(ordy), .r_addr(b_addr), .r_data(RdData)
			);

			always_ff @(posedge clk) begin
				if(ordy)  odat_ram[j] <= RdData;
			end
		end
		else begin : genBanked
			logic [N_RAM_BANKS-1:0][SIMD-1:0][WEIGHT_WIDTH-1:0]  BankRdData;
			...
			for(genvar k = 0; k < N_RAM_BANKS; k++) begin : genRam
				...
				assign  LocalWAddr = a_addr % BANK_DEPTH;
				assign  LocalRAddr = b_addr % BANK_DEPTH;

				local_weight_ram #(
					.DATA_WIDTH(WGT_WORD_BITS), .DEPTH(THIS_BANK_DEPTH), .RAM_STYLE(RAM_STYLE)
				) inst_ram (
					.clk(clk),
					.w_en(a_we[j] && (a_addr / BANK_DEPTH == k)),
					.w_addr(LocalWAddr), .w_data(a_data_in),
					.r_en(ordy && (b_addr / BANK_DEPTH == k)),
					.r_addr(LocalRAddr), .r_data(BankRdData[k])
				);
			end
			...
			always_ff @(posedge clk) begin
				if(ordy) begin
					ReadBank     <= b_addr / BANK_DEPTH;
					odat_ram[j] <= BankRdData[ReadBank];
				end
			end
		end
	end : genPe

endmodule : local_weight_buffer_single
...
module local_weight_buffer #(
	...
	if(N_BUFFERS == 1) begin : genSingleBuffer
		local_weight_buffer_single #(
			.PE(PE), .SIMD(SIMD), .WEIGHT_WIDTH(WEIGHT_WIDTH),
			.MH(MH), .MW(MW), .N_REPS(N_REPS), .DBG(DBG), .RAM_STYLE(RAM_STYLE)
		) inst (
			.clk(clk), .rst(rst),
	...
	else begin : genDoubleBuffer
		local_weight_buffer_double #(
			.PE(PE), .SIMD(SIMD), .WEIGHT_WIDTH(WEIGHT_WIDTH),
			.MH(MH), .MW(MW), .N_REPS(N_REPS), .DBG(DBG), .RAM_STYLE(RAM_STYLE)
		) inst (
			.clk(clk), .rst(rst),

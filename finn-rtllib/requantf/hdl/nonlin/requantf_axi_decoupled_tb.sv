 ...
 * @brief	Testbench for requantf_axi_decoupled.
 ***************************************************************************/

module requantf_axi_decoupled_tb;
`default_nettype none
	...
	for(genvar  t = 0; t < TEST_CNT; t++) begin : genTests
		...
		localparam int unsigned  P_WIDTH = PE*64;
		...
		requantf_axi_decoupled #(
			...
			.s_params_tready(prdy), .s_params_tvalid(pvld), .s_params_tdata(pdat),
		...
		initial begin
			...
			ordy = 0;
			$display(
				"[%0d] requantf_axi_decoupled test: N=%0d C=%0d throttle=%0b/%0b signed_out=%0d",
			...
			repeat(ROUNDS) begin
				...
				pvld <= 1;
				pdat <= {
					$shortrealtobits(BIASES[0][CntCF] + BIAS_ADJ),
					$shortrealtobits(SCALES[0][CntCF])
				};
...
endmodule : requantf_axi_decoupled_tb

 ...
 * @brief	Self-checking testbench for requantf_decoupled.
 ***************************************************************************/

module requantf_decoupled_tb;
`default_nettype none
	...
	for(genvar  i = 0; i < TEST_CNT; i++) begin : genDUTs
		...
		initial begin
			...
			$display(
				"[%0d] requantf_decoupled: N=%0d C=%0d PE=%0d throttled=%0d signed_out=%0d",
			...
			repeat(ROUNDS) begin
				...
				// Pack parameters: { bias'[31:0], scale[31:0] }
				ivld <= 1;
				idat <= x;
				foreach(pdat[pe]) begin
					pdat[pe][0] <= $shortrealtobits(SCALES[pe][cf]);
					pdat[pe][1] <= $shortrealtobits(BIASES[pe][cf] + BIAS_ADJ);
				end
				@(posedge clk);
				ivld <=  0;
				idat <= 'x;
...
endmodule : requantf_decoupled_tb

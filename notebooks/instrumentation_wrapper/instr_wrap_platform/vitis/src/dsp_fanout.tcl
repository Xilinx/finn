foreach inst [get_cells -hierarchical -filter {ORIG_REF_NAME == "mvu_vvu_8sx9_dsp58"}] {
	...
	set nets [get_nets parent_net -of_objects [get_pins -of_objects [get_cells "genDSPPE[*].genDSPChain[*].genDSP.DSP58_inst"] -filter {BUS_NAME == A || BUS_NAME == B}] -filter  {TYPE == SIGNAL}]

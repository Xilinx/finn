...
class UNSW_NB15_quantized_1(torch.utils.data.Dataset):
    def __init__(self, file_path_test, quantization=True, onehot=True, sequence_length=25, transform=None):   
        self.dataframe = pd.read_csv(file_path_test)
        ...
        #------------Apply Quantization------------
        if quantization:
            self.quantized_df = self.quantize_df(self.one_hot_encoded_df)
        else: 

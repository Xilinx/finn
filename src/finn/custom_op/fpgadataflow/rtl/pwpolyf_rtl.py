...
def _generate_coeffs_pkg_data(
    ...
    num_subs = 1 << K
    if partition_mode == "threshold":
        if partition_boundaries is None:
            partition_boundaries = _threshold_boundaries(K)
        partition_boundaries = _validate_threshold_boundaries(partition_boundaries)
        num_segs = int(partition_boundaries.size) + 1
    else:
    ...
    lines.append(
        "// DEGREE=%d K=%d NUM_OCTAVES=%d PARTITION_MODE=%s  Segments: %d"
        % (degree, K, NUM_OCTAVES, partition_mode, num_segs)
    )
    lines.append("")
    ...
    lines.append("    localparam int unsigned  NUM_THRESHOLDS = %d;" % (num_segs - 1))
    lines.append(
        "    localparam bit           USE_THRESHOLD_PARTITION = 1'b%d;"
        % (1 if partition_mode == "threshold" else 0)
    )
    lines.append("")
    lines.append("    localparam int unsigned  THRESHOLDS[NUM_THRESHOLDS] = '{")
    for idx, boundary in enumerate(partition_boundaries):
        comma = "," if idx < len(partition_boundaries) - 1 else ""
        lines.append("        32'h%s%s\t// threshold %d" % (_float_to_hex(boundary), comma, idx))
    lines.append("    };")
    lines.append("")
...
class PWPolyF_rtl(PWPolyF, RTLBackend):
    ...
    def _generate_coeffs_pkg(self, num_samples=1000):
        ...
        partition_mode = self.get_nodeattr("partitionMode")
        partition_boundaries = _parse_threshold_boundaries(self.get_nodeattr("partitionBoundaries"))

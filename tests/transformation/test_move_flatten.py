...
def test_move_flatten():
    inp = helper.make_tensor_value_info("inp", TensorProto.FLOAT, [1, 1024, 1, 1])
    ...
    # initialize values
    a0_values = np.random.uniform(low=0.1, high=0.99, size=(1))
    model.set_initializer("a0", a0_values)
    shape_values = np.asarray([1, -1]).astype(np.float32)
    ...
    a3_values = np.random.uniform(low=-1, high=1, size=(1000))
    model.set_initializer("a3", a3_values)
    model.set_initializer("k", np.asarray([5]))

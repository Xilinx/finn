...
DEFAULT_MODEL_PATH = os.path.join(HERE, "tinyvit_w4a6_qatfix_ds_finn_clean.onnx")
OUTPUT_DIR = os.path.join(HERE, "output_tinysiglip")
...
# MLO loops the single encoder-layer body once per layer (4 iterations/frame),
# and SetFolding budgets the body against target_cycles_per_frame WITHOUT dividing
# by iterations. So to hit ~50 FPS overall, fold the body to ~1/4 of the frame
# budget by targeting 50 * 4. The resulting per-node folding is written to
# output_tinysiglip/auto_folding_config.json (the effective single-layer config).
LOOP_ITERATIONS = 4
...
def select_build_steps():
    ...
    return [
        ...
        # (MLO: rolls the 4 encoder layers into one FINNLoop body).
